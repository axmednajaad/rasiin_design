 ...
 */
//  const DataTable = frappe.require('/assets/frappe/js/frappe/ui/datatable.js')
...
//  window.DataTable = DataTable;
 ...
 frappe.views.ReportView = class ReportView extends frappe.views.ListView {
	 ...
	 setup_view() {
		...
		// console.log("this is report view",frappe.listview_settings["Sales Invoice"])
	 ...
	 setup_page() {
		 ...
		 this.page.add_button("Print", () => {
			 // this.print_report();
			 this.table.print(false, true);
		 });
		 this.page.add_button("Export", () => {
			 this.table.download("xlsx", this.page_title +".xlsx", {sheetName:this.page_title ,  compress:false});
			 // this.export_report();
		 });
	 }
	 ...
	 setup_result_area() {
		 ...
		 this.$datatable_wrapper = $('<div class="datatable-wrapper" id = "example-table">');
	 ...
	 grouping_cols(){
		 ...
		 $('.groupcheck').change(function() {
			 ...
			 me.setup_datatable(me.data , true);
			 // setup_datatable()
	 ...
	 make_grouping_btn(){
		 ...
		 let me = this
			 this.columns.forEach(field => {
				 ...
				 if(field.docfield.fieldtype !== "Currency"){
					 ...
					 <li>
					 <input type="checkbox" class="form-check-input groupcheck ml-2"  value = '${field.docfield.fieldname}' >
		 ...
		 this.page.wrapper.find(".filter-selector").after(
		 ...
		 // </button>
			 ...
			 <button type="button" class="btn btn-default btn-sm"<a href="#" data-toggle="dropdown" class="dropdown-toggle">Summery by<b class="caret"></b></a>
			 </button>
	 ...
	 setup_datatable(values , gr_ref) {
		 ...
		 // 	],
		 // });
		 let doct = this.doctype.replace(' ' , '-').toLowerCase()
		 var tbldata = values
		 let me = this
		...
		if(!gr_ref){
			this.columns.unshift(
				...
				{formatter:"rowSelection", titleFormatter:"rowSelection", hozAlign:"left", headerSort:false, checked:function(e, cell){
				  ...
				  {title:"No", field:"no",type: "Field", formatter:"rownum"}
			)
			...
			this.columns.push(
				{
					...
					title : "Action",
					formatter:"html",
					// cellClick:function(e, cell){alert("cell click")}
				},
		...
		let list_btns = frappe.listview_settings[`${this.doctype}`]
		...
		tbldata.forEach(row => {
			...
			// let btnhml = `<button class='btn btn-default ml-2' onclick = "frappe.utils.print('${this.doctype}' , '${row.name}')"> <i class="action fa fa-print"></i></button>`
			...
			// 		if(row.status){
			// 		if(list_btns[key].status.includes(row.status)){
			// 		btnhml += `<button class='btn btn-${list_btns[key].color} ml-2' onclick = "${list_btns[key].action}"> ${list_btns[key].get_label()}</button>`
			...
			// 		btnhml += `<button class='action btn btn-${list_btns[key].color} ml-2' onclick = "frappe.utils.print('${this.doctype}' , '${row.name}')" > ${list_btns[key].get_label()}</button>`
		...
		// console.log(this.columns)
		 ...
		 this.table = new Tabulator(this.$datatable_wrapper[0], {
			// layout:"fitDataFill",
			...
			//  printHeader:`<img src = '/private/files/WhatsApp Image 2022-10-20 at 6.19.02 PM.jpeg'>`,
			 printFooter:"<h2>Patient Visit By Deparment<h2>",
			 // groupBy:"customer",
 ...
 // });
 // table.setGroupBy(table.columns.customer);
	 }
	 ...
	 set_fields() {
		...
		//  ["name", "docstatus"].map((f) => this._add_field(f));
		 ...
		 this.columns.forEach((f) => this._add_field(f.name, f.doc))
			 return;
	 ...
	 get_fields() {
		 ...
		 let fields = this.fields.map((f) => {
			...
			//  if (f[1] !== this.doctype) {
			// 	 // child table field
			// 	 column_name = frappe.model.get_full_column_name(f[0], f[1]);
			//  }
	 ...
	 build_column(c) {
		let  formatter = ""
			 ...
			 if (docfield) {
				 ...
				 if (fieldname == "docstatus" && !frappe.meta.has_field(this.doctype, "status")) {
					 ...
					 formatter = function(cell, formatterParams, onRendered){
						let sts = frappe.get_indicator(cell.getRow()._row.data ,myob.doctype);
						// console.log(cell)
						// cell._cell.element.style.backgroundColor = "#000";
						// return `<span style = "color: ${sts[1] }">${sts[0]}</span>`
						if(sts){
							return `<span id="badge" style = "background-color: ${sts[1]}  ">${sts[0]}</span>`
		 ...
		 if (docfield.fieldname === "creation"){
	 ...
	 get_checked_items(only_docnames) {
		 const items = this.table.getSelectedData();
		//  const items = indexes.map((i) => this.data.filter((dataele) => i.name == dataele.name));
		//  console.log(items)
		 if (only_docnames) {
